...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // kollar om headern inte är tom och har bearer prefix
        ...
        // SecurityContext används av Spring Security för att lagra information.
        // här kollar vi så att det inte redan finns en auktorisering i kontexten.
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {

            // vi laddar in användardata från databasen med hjälp av användarnamnet.
            UserDetails userDetails = this.userDetailsService.loadUserByUsername(username);

            // kontroller om token är giltig, signaturen korrekt och namn matchar användaren
            if (jwtUtil.isValid(jwt, username)) {

                // token som spring security använder för att representera en autentiserad
                // användare
